...
void visualize(void)
{
	...
	if (draw_vecs)
	{
        //Draw 'g' by showing cone-glyphs, scaled and rotated to reflect the
        //vector magnitude
        ...
        for (i = 0; i < DIM; i++)
            for (j = 0; j < DIM; j++)
            {
                float x_start = wn + (fftw_real)i * wn;
                float y_start = hn + (fftw_real)j * hn;
                float x_end = (wn + (fftw_real)i * wn) + vec_scale * vx[idx];
                float y_end = (hn + (fftw_real)j * hn) + vec_scale * vy[idx];
                float r, g, b;
                float length = sqrt(((x_start - x_end)*(x_start - x_end))+((y_start - y_end)*(y_start - y_end)));
                r = g = b = vy;
                glColor3f(r,g,b);
                
                float radius = length*0.6;
                float angle = direction_to_angle(x_start, y_start, x_end, y_end);
            }
...
static float direction_to_angle(x_start, y_start, x_end, y_end) {
    cosa
}
